...
class spiders(threading.Thread):
    ...
    def request_url(self, url):     # 爬取并存储每一页的图片
        ...
        urls = html1.xpath("//figure/img/@data-src")  # 获取到跳转页面
        urlsLength = len(urls)
        if not urlsLength == 0:
            ...
            for i in range(urlsLength):
                ...
                if os.path.exists(folder + '/' + file_name):
                    continue
                try:
                    dowmloadPath = 'https://w.wallhaven.cc/full/' + urls[i].split('/')[-2]+ '/wallhaven-' + file_name
                    responsegraph = requests.get(dowmloadPath, headers=headers1, timeout=5000)
